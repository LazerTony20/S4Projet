...
{
	socketSendTo(context.socket, &ipAddr, port, Send_Moyenne, 3, NULL, 0);
	return 1;
}

int udpEchoTask(u32 *inBuffer)
{
   //Main loop
   {
	   int i = 0;
      ...
      if(!error)
      {
         //Debug messages
    	  ...
    	  for(i = 0 ; i < 128 ; i++)
    	  {
    		  inBuffer[i] = ((((u32)context.buffer[2*i] & 0x000000FF) << 8) + ((u32)context.buffer[2*i+1] & 0x000000FF)) & 0x0000FFFF;
    	  }
