...
void ACL_Init()
{
    ...
    ACL_SetRegister(ACL_CTRL_REG1, 0x18); // Output data rate at 800Hz, no auto wake, no auto scale adjust, no fast read mode
    ACL_SetRegister(ACL_CTRL_REG1, 0x19); // Output data rate at 800Hz, no auto wake, no auto scale adjust, no fast read mode
}
...
unsigned char ACL_GetRegister(unsigned char bAddress)
{
    ...
    if (I2C_Write(ACL_I2C_ADDR, &bAddress, 1, 0))
        return 0xFF;
