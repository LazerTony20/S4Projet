...
void sendTask ()
{
    TCPIP_UDP_ArrayPut(appData.clientSocket, (uint8_t*)UDP_Send_Buffer, 256);
...
void _UDP_ClientTasks() {
    ...
    switch(appData.clientState) {
        case UDP_TCPIP_WAITING_FOR_COMMAND: {
            SYS_CMD_READY_TO_READ();
                ...
                if (result == TCPIP_DNS_RES_NAME_IS_IPADDRESS) {
                    IPV4_ADDR addr;
                    TCPIP_Helper_StringToIPAddress("192.168.13.2", &addr);
        ...
        case UDP_TCPIP_WAIT_FOR_CONNECTION: {
            ...
            TCPIP_UDP_ArrayPut(appData.clientSocket, (uint8_t*)UDP_Send_Buffer, 64);
//            {
//                SYS_CONSOLE_MESSAGE ("yallah \r\n");
//            };
...
//            }
            ...
            if (UDP_bytes_received) {
                TCPIP_UDP_ArrayGet(appData.clientSocket, (uint8_t*)UDP_Receive_Buffer, 3);
                SYS_CONSOLE_MESSAGE("salut \r\n");
                ...
                reception[2] = UDP_Receive_Buffer[2];
